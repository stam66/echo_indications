...
Protected Module AuditTracker
	#tag Method, Flags = &h0
		Function LogCreate(db As MySQLCommunityServer, tableName As String, primaryKey As Integer, username As String, newData As Dictionary) As Boolean
		  // db is now an explicit parameter — the audit log used to reach for
		  // Session.DB, but API requests have no WebSession, so callers must
		  // pass in whichever connection they're using.
		  Var sql As String
	...
	#tag Method, Flags = &h0
		Function LogDelete(db As MySQLCommunityServer, tableName As String, primaryKey As Integer, username As String, oldData As Dictionary) As Boolean
		  Var sql As String
	...
	#tag Method, Flags = &h0
		Function LogUpdate(db As MySQLCommunityServer, tableName As String, primaryKey As Integer, username As String, oldData As Dictionary, newData As Dictionary) As Boolean
