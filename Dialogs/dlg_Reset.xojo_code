...
#tag WindowCode
	#tag Method, Flags = &h0
		Sub resetPassword(email as string)
		  if not app.isValidEmail(email) then
		    MessageBox("Password reset: The email entered is not valid. Please correct this and try again.")
		    Return
		  end If
		  
		  var sql as string = "select * from users where email = ?"
		  ...
		  while not rs.AfterLastRow
		    if rs.Column("email").StringValue = email then
		      ...
		      app.SendMail(email, "Your one-time password for ECHOIndications.org", "Your one-time password is:      " + tempPass + EndOfLine + EndOfLine + "After logging in with this password you will be asked to create a new password.", Session)
